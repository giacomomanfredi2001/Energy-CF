function err = MAE(p, F_params, prices, T1, T2, m, n, X_t, Y_t)
% This function calculates the Mean Absolute Error (MAE) between the
...
%
% Returns:
% err       The mean absolute error between model-predicted and observed
%           prices

% Calculate the MAE by iterating over all Future contracts
err = mean(abs(arrayfun(@(i) F_params(0,T1(i),T2(i),p,m,n,X_t,Y_t)-prices(i),...
                    1:length(T1))'));
